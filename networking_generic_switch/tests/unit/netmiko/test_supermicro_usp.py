...
from networking_generic_switch.devices.netmiko_devices import supermicro
from networking_generic_switch.tests.unit.netmiko import test_netmiko_base


class TestNetmikoSupermicroUsp(test_netmiko_base.NetmikoSwitchTestBase):

    def _make_switch_device(self):
        device_cfg = {'device_type': 'netmiko_supermicro_usp'}
        return supermicro.SupermicroUsp(device_cfg)
    ...
    def test__format_commands(self):
        cmd_set = self.switch._format_commands(
            supermicro.SupermicroUsp.ADD_NETWORK,
            segmentation_id=22,
            network_id=22,
        ...
        cmd_set = self.switch._format_commands(
            supermicro.SupermicroUsp.DELETE_NETWORK,
            segmentation_id=22)
        self.assertEqual(cmd_set, ['no vlan 22'])

        cmd_set = self.switch._format_commands(
            supermicro.SupermicroUsp.PLUG_PORT_TO_NETWORK,
            port=3333,
            segmentation_id=33)
        ...
        cmd_set = self.switch._format_commands(
            supermicro.SupermicroUsp.DELETE_PORT,
            port=3333,
            segmentation_id=33)
